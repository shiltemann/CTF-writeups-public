...
public final class Detector {
    ...
    private static final class Point {
        public final int f2x;
        public final int f3y;

        public ResultPoint toResultPoint() {
            return new ResultPoint((float) this.f2x, (float) this.f3y);
        }
    ...
    private ResultPoint[] getMatrixCornerPoints(Point[] bullEyeCornerPoints) throws NotFoundException {
        ...
        int dx = bullEyeCornerPoints[0].f2x - bullEyeCornerPoints[2].f2x;
        dx += dx > 0 ? 1 : -1;
        int dy = bullEyeCornerPoints[0].f3y - bullEyeCornerPoints[2].f3y;
        dy += dy > 0 ? 1 : -1;
        int targetcx = MathUtils.round(((float) bullEyeCornerPoints[2].f2x) - (((float) dx) * ratio));
        int targetcy = MathUtils.round(((float) bullEyeCornerPoints[2].f3y) - (((float) dy) * ratio));
        int targetax = MathUtils.round(((float) bullEyeCornerPoints[0].f2x) + (((float) dx) * ratio));
        int targetay = MathUtils.round(((float) bullEyeCornerPoints[0].f3y) + (((float) dy) * ratio));
        dx = bullEyeCornerPoints[1].f2x - bullEyeCornerPoints[3].f2x;
        dx += dx > 0 ? 1 : -1;
        dy = bullEyeCornerPoints[1].f3y - bullEyeCornerPoints[3].f3y;
        dy += dy > 0 ? 1 : -1;
        int targetdx = MathUtils.round(((float) bullEyeCornerPoints[3].f2x) - (((float) dx) * ratio));
        int targetdy = MathUtils.round(((float) bullEyeCornerPoints[3].f3y) - (((float) dy) * ratio));
        int targetbx = MathUtils.round(((float) bullEyeCornerPoints[1].f2x) + (((float) dx) * ratio));
        int targetby = MathUtils.round(((float) bullEyeCornerPoints[1].f3y) + (((float) dy) * ratio));
    ...
    private Point[] getBullEyeCornerPoints(Point pCenter) throws NotFoundException {
        ...
        int dx = pina.f2x - pinc.f2x;
        int dy = pina.f3y - pinc.f3y;
        int targetcx = MathUtils.round(((float) pinc.f2x) - (((float) dx) * ratio));
        int targetcy = MathUtils.round(((float) pinc.f3y) - (((float) dy) * ratio));
        int targetax = MathUtils.round(((float) pina.f2x) + (((float) dx) * ratio));
        int targetay = MathUtils.round(((float) pina.f3y) + (((float) dy) * ratio));
        dx = pinb.f2x - pind.f2x;
        dy = pinb.f3y - pind.f3y;
        int targetdx = MathUtils.round(((float) pind.f2x) - (((float) dx) * ratio));
        int targetdy = MathUtils.round(((float) pind.f3y) - (((float) dy) * ratio));
        int targetbx = MathUtils.round(((float) pinb.f2x) + (((float) dx) * ratio));
        int targetby = MathUtils.round(((float) pinb.f3y) + (((float) dy) * ratio));
        if (isValid(targetax, targetay) && isValid(targetbx, targetby) && isValid(targetcx, targetcy) && isValid(targetdx, targetdy)) {
    ...
    private boolean[] sampleLine(Point p1, Point p2, int size) {
        ...
        float dx = (((float) (p2.f2x - p1.f2x)) * moduleSize) / d;
        float dy = (((float) (p2.f3y - p1.f3y)) * moduleSize) / d;
        float px = (float) p1.f2x;
        float py = (float) p1.f3y;
    ...
    private boolean isWhiteOrBlackRectangle(Point p1, Point p2, Point p3, Point p4) {
        Point p12 = new Point(p1.f3y + 3, null);
        Point p22 = new Point(p2.f3y - 3, null);
        Point p32 = new Point(p3.f3y - 3, null);
        Point p42 = new Point(p4.f3y + 3, null);
    ...
    private int getColor(Point p1, Point p2) {
        float d = distance(p1, p2);
        float dx = ((float) (p2.f2x - p1.f2x)) / d;
        float dy = ((float) (p2.f3y - p1.f3y)) / d;
        int error = 0;
        float px = (float) p1.f2x;
        float py = (float) p1.f3y;
        boolean colorModel = this.image.get(p1.f2x, p1.f3y);
    ...
    private static float distance(Point a, Point b) {
        return MathUtils.distance(a.f2x, a.f3y, b.f2x, b.f3y);
    }
}
